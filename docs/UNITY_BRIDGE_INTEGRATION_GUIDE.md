# 🔌 Unity Bridge - AI Tool Integration Guide

**Version:** 1.0.0  
**For:** Unity Bridge v1.0.0  
...
This guide explains how to integrate various AI tools with Unity Bridge to enable direct Unity manipulation.
...
1. **AI writes** to `unity_bridge_commands.json`
2. **Unity reads** and executes
3. **Unity writes** to `unity_bridge_results.json`
4. **AI reads** result

### Protocol Specification

#### Command File Format
**Location:** `<UnityProjectRoot>/unity_bridge_commands.json`
...
#### Result File Format
**Location:** `<UnityProjectRoot>/unity_bridge_results.json`
...
#### Log File Format
**Location:** `<UnityProjectRoot>/unity_bridge_logs.txt`
...
3. Unity Bridge files are in scope
...
class UnityBridge:
    def __init__(self, project_path):
        ...
        self.command_file = self.project_path / "unity_bridge_commands.json"
        self.result_file = self.project_path / "unity_bridge_results.json"
...
# Usage
bridge = UnityBridge("C:/Projects/MyUnityGame")
...
class UnityBridgeClient:
    """Python client for Unity Bridge communication."""
    ...
    def __init__(self, project_path: str, polling_interval: float = 1.0):
        ...
        self.command_file = self.project_path / "unity_bridge_commands.json"
        self.result_file = self.project_path / "unity_bridge_results.json"
        self.log_file = self.project_path / "unity_bridge_logs.txt"
        
        # Verify Unity Bridge is active
        if not self._is_bridge_active():
            raise RuntimeError("Unity Bridge not active. Start Unity first.")
    
    def _is_bridge_active(self) -> bool:
        """Check if Unity Bridge is responding."""
        try:
            result = self.ping()
...
if __name__ == "__main__":
    # Connect to Unity
    bridge = UnityBridgeClient("C:/Projects/MyUnityGame")
...
class UnityBridgeClient {
    constructor(projectPath, pollingInterval = 1000) {
        ...
        this.commandFile = path.join(projectPath, 'unity_bridge_commands.json');
        this.resultFile = path.join(projectPath, 'unity_bridge_results.json');
        this.logFile = path.join(projectPath, 'unity_bridge_logs.txt');
    }
...
// Usage Example
async function main() {
    const bridge = new UnityBridgeClient('C:/Projects/MyUnityGame');
...
impl UnityBridgeClient {
    pub fn new(project_path: &str, polling_interval_ms: u64) -> Self {
        let project_path = PathBuf::from(project_path);
        let command_file = project_path.join("unity_bridge_commands.json");
        let result_file = project_path.join("unity_bridge_results.json");
...
// Usage Example
fn main() {
    let bridge = UnityBridgeClient::new("C:/Projects/MyUnityGame", 1000);
...
const UnityBridgeClient = require('./unity-bridge-client');
...
app.listen(3000, () => {
    console.log('Unity Bridge API running on http://localhost:3000');
});
```
...
bridge = UnityBridgeClient(project_path, polling_interval=0.25)
```
...
def safe_execute(bridge, command_type, parameters) -> Optional[Dict]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"⚠️ Unity Bridge files not found. Is Unity running?")
...
- **Full Documentation:** [UNITY_BRIDGE_GUIDE.md](UNITY_BRIDGE_GUIDE.md)
- **Quick Reference:** [UNITY_BRIDGE_QUICK_REFERENCE.md](UNITY_BRIDGE_QUICK_REFERENCE.md)
...
Unity Bridge enables universal AI-Unity integration through simple file-based communication. Any AI tool or programming language with file I/O can control Unity directly.
