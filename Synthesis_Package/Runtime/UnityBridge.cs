    ...
    public class UnityBridge : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (enableKnowledgeBase)
            {
                try
                {
                    knowledgeBase = new SynthesisKnowledgeBase();
                    knowledgeBase.Initialize(string.IsNullOrEmpty(knowledgeBasePath) ? null : knowledgeBasePath);
                    Log("📚 Synthesis Knowledge Base Initialized!");
                }
                catch (System.Exception ex)
                {
                    Log($"⚠️ Knowledge Base initialization failed: {ex.Message}");
                }
            }
            #endif
        ...
        private void OnDestroy()
        {
            ...
            if (knowledgeBase != null && knowledgeBase.IsInitialized)
            {
                knowledgeBase.Close();
            }
            #endif
        ...
        private void ExecuteCommand(BridgeCommand cmd)
        {
            ...
            try
            {
                ...
                switch (cmd.type)
                {
                    ...
                    case "SearchCommands":
                        result = SearchCommandsCommand(cmd);
                        break;
                    
                    case "SearchWorkflows":
                        result = SearchWorkflowsCommand(cmd);
                        break;
                    
                    case "SearchFAQ":
                        result = SearchFAQCommand(cmd);
                        break;
        ...
        #if UNITY_EDITOR
        private BridgeResult SearchCommandsCommand(BridgeCommand cmd)
        {
            var result = new BridgeResult { commandId = cmd.id };
            
            if (!enableKnowledgeBase || knowledgeBase == null || !knowledgeBase.IsInitialized)
            {
                result.success = false;
                result.message = "Knowledge Base is not enabled or initialized";
                return result;
            }
            
            if (!cmd.parameters.ContainsKey("searchTerm"))
            {
                result.success = false;
                result.message = "Missing required parameter: searchTerm";
                return result;
            }
            
            string searchTerm = cmd.parameters["searchTerm"].ToString();
            var commands = knowledgeBase.SearchCommands(searchTerm);
            
            result.success = true;
            result.message = $"Found {commands.Count} command(s)";
        ...
        private BridgeResult SearchWorkflowsCommand(BridgeCommand cmd)
        {
            var result = new BridgeResult { commandId = cmd.id };
            ...
            if (!cmd.parameters.ContainsKey("tag"))
            {
                result.success = false;
                result.message = "Missing required parameter: tag";
                return result;
            }
            
            string tag = cmd.parameters["tag"].ToString();
            var workflows = knowledgeBase.SearchWorkflows(tag);
            
            result.success = true;
            result.message = $"Found {workflows.Count} workflow(s)";
            result.data = new Dictionary<string, object>
            {
                { "workflows", workflows },
        ...
        private BridgeResult SearchFAQCommand(BridgeCommand cmd)
        {
            var result = new BridgeResult { commandId = cmd.id };
            ...
            string searchTerm = cmd.parameters["searchTerm"].ToString();
            var faqs = knowledgeBase.SearchFAQ(searchTerm);
            
            result.success = true;
            result.message = $"Found {faqs.Count} FAQ(s)";
            result.data = new Dictionary<string, object>
            {
                { "faqs", faqs },
                { "count", faqs.Count }
