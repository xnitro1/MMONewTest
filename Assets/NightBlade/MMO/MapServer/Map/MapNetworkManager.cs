...
namespace NightBlade.MMO
{
    ...
    public partial class MapNetworkManager : BaseGameNetworkManager, IAppServer
    {
        ...
        [HideInInspector] public bool centralConnectionFoldout = true;
        [HideInInspector] public bool mapSpawnFoldout = true;
        [HideInInspector] public bool playerDisconnectionFoldout = false;
        ...
        // TEMPORARY: Disable auto-quit for testing
        // TODO: Remove this after testing! Re-enable auto-quit for production!
#if (UNITY_EDITOR || UNITY_SERVER || !EXCLUDE_SERVER_CODES) && UNITY_STANDALONE
        [SerializeField]
        [Tooltip("TEMPORARY: Disable auto-quit for testing. MUST re-enable for production!")]
#endif
        ...
        protected override void Awake()
        {
            ...
            lock (_instanceLock)
            {
                ...
                if (_instanceCount > 1)
                {
                    ...
                    Logging.LogError($"⚠️ Current instance: {gameObject.name} on scene {UnityEngine.SceneManagement.SceneManager.GetActiveScene().name}");
                    
                    // In editor, also log to Unity console for visibility
#if UNITY_EDITOR
                    Debug.LogError($"⚠️ CRITICAL: {_instanceCount} MapNetworkManager instances! Static state will corrupt data! See docs/Instance_Based_Server_Architecture.md", this);
#endif
                }
                else
                {
            ...
            // Initialize MapAssetManager for memory optimization
            gameObject.GetOrAddComponent<NightBlade.MMO.MapServer.Map.MapAssetManager>();
#endif
            base.Awake();
        }
...
#if (UNITY_EDITOR || UNITY_SERVER || !EXCLUDE_SERVER_CODES) && UNITY_STANDALONE
            if (ClusterClient.IsNetworkActive && _usersByCharacterId.ContainsKey(playerCharacterEntity.Id))
            {
                // Thread-safe update using AddOrUpdate
                SocialCharacterData tempUserData = SocialCharacterData.Create(playerCharacterEntity);
        ...
        public async void ProceedBeforeQuit()
        {
            ...
            // TEMPORARY: Skip auto-quit if disabled for testing
            if (disableAutoQuit)
            {
                Logging.Log("[MapNetworkManager] Auto-quit disabled (TESTING MODE - re-enable for production!)");
                ReadyToQuit = false;
                return;
            }
            ...
            {
                Logging.LogError($"[MapNetworkManager] Error in ProceedBeforeQuit: {ex.Message}\n{ex.StackTrace}");
                ReadyToQuit = true; // Allow quit even on error to prevent hanging
            }
        }
        ...
        {
            // Set user data to map server - TryAdd is atomic and thread-safe
            SocialCharacterData userData = SocialCharacterData.Create(playerCharacterEntity);
            if (_usersByCharacterId.TryAdd(userData.id, userData))
            {
                // Add map user to cluster server if added successfully
        ...
        public override async void OnPeerDisconnected(long connectionId, DisconnectReason reason, SocketError socketError)
        {
            ...
            {
                Logging.LogError($"[MapNetworkManager] Error in OnPeerDisconnected for connection {connectionId}: {ex.Message}\n{ex.StackTrace}");
            }
        }
#endif
        ...
        public override async UniTask<bool> DeserializeEnterGameData(uint requestId, long connectionId, EnterGameRequestMessage request, NetDataReader reader)
        {
            ...
            MapInfo.GetEnterMapPoint(playerCharacterData, out string mapName, out Vector3 position, out Vector3 rotation);
        ...
        protected override async void HandleChatAtServer(MessageHandlerData messageHandler)
        {
            ...
            {
                Logging.LogError($"[MapNetworkManager] Error in HandleChatAtServer: {ex.Message}\n{ex.StackTrace}");
            }
        }
#endif
        ...
        private async void OnPlayerCharacterRemoved(string userId, string characterId)
        {
            ...
            {
                Logging.LogError($"[MapNetworkManager] Error in OnPlayerCharacterRemoved for user {userId}: {ex.Message}\n{ex.StackTrace}");
            }
        }
#endif
        #endregion
            ...
            RequestProceedResultDelegate<ResponseSpawnMapMessage> result)
        {
            ...
            if (MapInfo == null || !string.Equals(MapInfo.Id, request.mapName))
            {
                result.InvokeError(new ResponseSpawnMapMessage()
                {
