...
namespace NightBlade.MMO
{
    public class UIMmoSceneHome : UIHistory
    {
        ...
        {
            // Ensure MMOClientInstance exists
            if (MMOClientInstance.Singleton == null)
            {
                Debug.LogWarning("[UIMmoSceneHome] MMOClientInstance singleton is null, creating new instance");
                // The singleton should be created automatically, but let's ensure it exists
                return;
            }
        ...
        public void OnCentralServerConnected()
        {
            ClearHistory();
            Next(uiLogin);
            if (!string.IsNullOrEmpty(GameInstance.UserId) && !string.IsNullOrEmpty(GameInstance.AccessToken) && MMOClientInstance.Singleton != null)
