...
namespace NightBlade.MMO
{
    public partial class CentralNetworkManager
    {
            ...
            RequestProceedResultDelegate<ResponseSelectCharacterMessage> result)
        {
#if NET || NETCOREAPP || ((UNITY_EDITOR || UNITY_SERVER || !EXCLUDE_SERVER_CODES) && UNITY_STANDALONE)
            long connectionId = requestHandler.ConnectionId;
            Debug.Log($"[HandleRequestSelectCharacter] Processing request for character {request.characterId} in channel {request.channelId}");
            if (!_userPeers.TryGetValue(connectionId, out CentralUserPeerInfo userPeerInfo))
            {
            ...
            // Direct lookup like R2 - build key and check dictionary
        ...
        // CCU SYSTEM REMOVED - TryCreateAndAssignInstanceForChannel method deleted
    }
}
