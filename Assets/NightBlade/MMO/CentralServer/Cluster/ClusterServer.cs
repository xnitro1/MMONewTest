...
namespace NightBlade.MMO
{
    public class ClusterServer : LiteNetLibServer
    {
        ...
        // Event for when instance servers register
        public event System.Action<CentralServerPeerInfo, string> OnInstanceServerRegistered;
                ...
                switch (request.peerInfo.peerType)
                {
                    ...
                    case CentralServerPeerType.InstanceMapServer:
                        if (!_instanceMapServerPeersByKey.ContainsKey(key))
                        {
                            ...
                            // Notify listeners about instance registration
                            OnInstanceServerRegistered?.Invoke(peerInfo, key);
            ...
            RequestProceedResultDelegate<ResponseSpawnMapMessage> result)
        {
            ...
            // Check if there are any map spawn servers available
            if (connectionIds.Count == 0)
            {
                Logging.LogError(LogTag, $"No map spawn servers available to spawn map: {request.mapName}");
