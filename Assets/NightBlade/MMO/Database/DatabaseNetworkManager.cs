...
namespace NightBlade.MMO
{
    ...
    public partial class DatabaseNetworkManager : LiteNetLibManager.LiteNetLibManager
    {
        ...
        // When false, server won't auto-quit when clients disconnect
#if (UNITY_EDITOR || UNITY_SERVER || !EXCLUDE_SERVER_CODES) && UNITY_STANDALONE
        [SerializeField]
        [Tooltip("Disable auto-quit for testing ONLY. MUST be FALSE for production! Default: false")]
#endif
        private bool disableAutoQuit = false; // Default: false for production. Set to true only for testing.
        ...
        public async void ProceedBeforeQuit()
        {
            ...
            if (disableAutoQuit)
            {
                Logging.Log("[DatabaseNetworkManager] Auto-quit disabled (TESTING MODE - re-enable for production!)");
                ReadyToQuit = false;
                return;
            }
