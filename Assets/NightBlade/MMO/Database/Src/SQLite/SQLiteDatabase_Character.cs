    ...
    public partial class SQLiteDatabase
    {
        ...
        public override UniTask<List<SocialCharacterData>> FindCharacters(string finderId, string characterName, int skip, int limit)
        {
            ...
            ExecuteReader((reader) =>
            {
                ...
                new SqliteParameter("@finderId", finderId));

            // Build parameterized exclude query
            List<SqliteParameter> parameters = new List<SqliteParameter>();
            parameters.Add(new SqliteParameter("@characterName", "%" + characterName + "%"));
            ...
            for (int i = 0; i < excludeIds.Count; i++)
            {
                ...
                parameters.Add(new SqliteParameter(paramName, excludeIds[i]));
            }
            ...
            }, $"SELECT id, dataId, characterName, level FROM characters WHERE characterName LIKE @characterName AND ({excludeIdsClause}) LIMIT @limit OFFSET @skip",
                parameters.ToArray());
