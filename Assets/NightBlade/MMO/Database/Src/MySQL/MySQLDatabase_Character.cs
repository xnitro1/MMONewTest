    ...
    public partial class MySQLDatabase
    {
...
#endif
                result.Reputation = reader.GetInt32(33);
                result.LastDeadTime = reader.GetInt64(34);
                result.UnmuteTime = reader.GetInt64(35);
                result.LastUpdate = ((System.DateTimeOffset)System.DateTime.SpecifyKind(reader.GetDateTime(36), System.DateTimeKind.Utc)).ToUnixTimeSeconds();
#if !DISABLE_CLASSIC_PK
                if (!reader.IsDBNull(37))
                    result.IsPkOn = reader.GetBoolean(37);
                if (!reader.IsDBNull(38))
                    result.LastPkOnTime = reader.GetInt64(38);
                if (!reader.IsDBNull(39))
                    result.PkPoint = reader.GetInt32(39);
                if (!reader.IsDBNull(40))
                    result.ConsecutivePkKills = reader.GetInt32(40);
                if (!reader.IsDBNull(41))
                    result.HighestPkPoint = reader.GetInt32(41);
                if (!reader.IsDBNull(42))
                    result.HighestConsecutivePkKills = reader.GetInt32(42);
#endif
            ...
            bool withPublicCustomData = true)
        {
            ...
            await ExecuteReader((reader) =>
            {
                ...
                c.currentChannel,
                c.currentMapName, c.currentPositionX, c.currentPositionY, c.currentPositionZ, c.currentRotationX, c.currentRotationY, c.currentRotationZ,
                c.currentSafeArea,
                c.respawnMapName, c.respawnPositionX, c.respawnPositionY, c.respawnPositionZ,
                c.reputation, c.lastDeadTime, c.unmuteTime, c.updateAt,
        ...
        public override async UniTask<List<PlayerCharacterData>> GetCharacters(string userId)
        {
            ...
            // Single query to get all characters with their base data + mount + pk info
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    PlayerCharacterData character;
                    if (GetCharacter(reader, out character))
                    {
                ...
                LEFT JOIN character_pk AS cpk ON c.id = cpk.id
                LEFT JOIN charactermount AS cmnt ON c.id = cmnt.id
                WHERE c.userId=@userId ORDER BY c.updateAt DESC",
                new MySqlParameter("@userId", userId));
            ...
            // For character list, we need: equip weapons, equip items, and public custom data
            List<UniTask> tasks = new List<UniTask>();
            tasks.Add(ReadCharacterEquipWeaponsBatch(characterIds, characterDict));
            tasks.Add(ReadCharacterEquipItemsBatch(characterIds, characterDict));
#if !DISABLE_CUSTOM_CHARACTER_DATA
            tasks.Add(ReadCharacterPublicDataBatch(characterIds, characterDict));
#endif
            await UniTask.WhenAll(tasks);
        ...
        private async UniTask ReadCharacterEquipWeaponsBatch(List<string> characterIds, Dictionary<string, PlayerCharacterData> characterDict)
        {
            ...
            for (int i = 0; i < characterIds.Count; i++)
            {
                ...
                parameters.Add(new MySqlParameter(paramName, characterIds[i]));
            }
            ...
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    string odCharacterId = reader.GetString(0);
                    if (characterDict.TryGetValue(odCharacterId, out PlayerCharacterData character))
                    {
                        EquipWeapons equipWeapons = new EquipWeapons();
                        int odIdx = reader.GetInt32(1);
                        CharacterItem rightHand = new CharacterItem();
                        CharacterItem leftHand = new CharacterItem();
                        
                        if (!reader.IsDBNull(2)) rightHand.dataId = reader.GetInt32(2);
                        if (!reader.IsDBNull(3)) rightHand.level = reader.GetInt32(3);
                        if (!reader.IsDBNull(4)) rightHand.amount = reader.GetInt32(4);
                        if (!reader.IsDBNull(5)) rightHand.durability = reader.GetFloat(5);
                        if (!reader.IsDBNull(6)) rightHand.exp = reader.GetInt32(6);
                        if (!reader.IsDBNull(7)) rightHand.lockRemainsDuration = reader.GetFloat(7);
                        if (!reader.IsDBNull(8)) rightHand.ammo = reader.GetInt32(8);
                        if (!reader.IsDBNull(9)) rightHand.ReadSockets(reader.GetString(9));

                        if (!reader.IsDBNull(10)) leftHand.dataId = reader.GetInt32(10);
                        if (!reader.IsDBNull(11)) leftHand.level = reader.GetInt32(11);
                        if (!reader.IsDBNull(12)) leftHand.amount = reader.GetInt32(12);
                        if (!reader.IsDBNull(13)) leftHand.durability = reader.GetFloat(13);
                        if (!reader.IsDBNull(14)) leftHand.exp = reader.GetInt32(14);
                        if (!reader.IsDBNull(15)) leftHand.lockRemainsDuration = reader.GetFloat(15);
                        if (!reader.IsDBNull(16)) leftHand.ammo = reader.GetInt32(16);
                        if (!reader.IsDBNull(17)) leftHand.ReadSockets(reader.GetString(17));

                        equipWeapons.rightHand = rightHand;
                        equipWeapons.leftHand = leftHand;

                        // Ensure the list is big enough
                        while (character.SelectableWeaponSets.Count <= odIdx)
                            character.SelectableWeaponSets.Add(new EquipWeapons());
                        character.SelectableWeaponSets[odIdx] = equipWeapons;
                    }
                }
            }, $@"SELECT characterId, idx,
                rightHandDataId, rightHandLevel, rightHandAmount, rightHandDurability, rightHandExp, rightHandLockRemainsDuration, rightHandAmmo, rightHandSockets,
                leftHandDataId, leftHandLevel, leftHandAmount, leftHandDurability, leftHandExp, leftHandLockRemainsDuration, leftHandAmmo, leftHandSockets
                FROM characteritem WHERE inventoryType IN (@invTypeRight, @invTypeLeft) AND characterId IN ({inClause}) ORDER BY characterId, idx",
                parameters.Concat(new[] { 
                    new MySqlParameter("@invTypeRight", (byte)InventoryType.EquipWeaponRight),
                    new MySqlParameter("@invTypeLeft", (byte)InventoryType.EquipWeaponLeft)
                }).ToArray());
        }

        private async UniTask ReadCharacterEquipItemsBatch(List<string> characterIds, Dictionary<string, PlayerCharacterData> characterDict)
        {
            if (characterIds.Count == 0) return;
            ...
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    ...
                    if (characterDict.TryGetValue(odCharacterId, out PlayerCharacterData character))
                    {
                        ...
                        item.dataId = reader.GetInt32(1);
                        item.level = reader.GetInt32(2);
                        item.amount = reader.GetInt32(3);
                        item.equipSlotIndex = reader.GetByte(4);
                        item.durability = reader.GetFloat(5);
                        item.exp = reader.GetInt32(6);
                        item.lockRemainsDuration = reader.GetFloat(7);
                        item.ammo = reader.GetInt32(8);
                        if (!reader.IsDBNull(9))
                            item.ReadSockets(reader.GetString(9));
                        character.EquipItems.Add(item);
                    }
                }
            }, $@"SELECT characterId, dataId, level, amount, equipSlotIndex, durability, exp, lockRemainsDuration, ammo, sockets
                FROM characteritem WHERE inventoryType=@invType AND characterId IN ({inClause}) ORDER BY characterId, idx",
                parameters.Concat(new[] { new MySqlParameter("@invType", (byte)InventoryType.EquipItems) }).ToArray());
        }

#if !DISABLE_CUSTOM_CHARACTER_DATA
        private async UniTask ReadCharacterPublicDataBatch(List<string> characterIds, Dictionary<string, PlayerCharacterData> characterDict)
        {
            if (characterIds.Count == 0) return;
            ...
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    ...
                    if (characterDict.TryGetValue(odCharacterId, out PlayerCharacterData character))
                    {
                        CharacterDataBoolean data = new CharacterDataBoolean();
                        data.hashedKey = reader.GetInt32(1);
                        data.value = reader.GetBoolean(2);
                        character.PublicBools.Add(data);
                    }
                }
            }, $"SELECT characterId, hashedKey, value FROM character_public_boolean WHERE characterId IN ({inClause})",
                parameters.ToArray());
            ...
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    ...
                    if (characterDict.TryGetValue(odCharacterId, out PlayerCharacterData character))
                    {
                        CharacterDataInt32 data = new CharacterDataInt32();
                        data.hashedKey = reader.GetInt32(1);
                        data.value = reader.GetInt32(2);
                        character.PublicInts.Add(data);
                    }
                }
            }, $"SELECT characterId, hashedKey, value FROM character_public_int32 WHERE characterId IN ({inClause})",
                parameters.ToArray());

            // Load public float32s
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    ...
                    if (characterDict.TryGetValue(odCharacterId, out PlayerCharacterData character))
                    {
                        CharacterDataFloat32 data = new CharacterDataFloat32();
                        data.hashedKey = reader.GetInt32(1);
                        data.value = reader.GetFloat(2);
                        character.PublicFloats.Add(data);
                    }
                }
            }, $"SELECT characterId, hashedKey, value FROM character_public_float32 WHERE characterId IN ({inClause})",
                parameters.ToArray());
        }
#endif
        ...
        public override async UniTask<List<SocialCharacterData>> FindCharacters(string finderId, string characterName, int skip, int limit)
        {
            ...
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    excludeIds.Add(reader.GetString(0));
                }
            }, "SELECT characterId2 FROM friend WHERE characterId1=@finderId",
                new MySqlParameter("@finderId", finderId));

            // Build parameterized exclude query
            List<MySqlParameter> parameters = new List<MySqlParameter>();
            parameters.Add(new MySqlParameter("@characterName", "%" + characterName + "%"));
            parameters.Add(new MySqlParameter("@skip", skip));
            parameters.Add(new MySqlParameter("@limit", limit));

            // Build exclude IDs clause with parameters
            List<string> excludeConditions = new List<string>();
            for (int i = 0; i < excludeIds.Count; i++)
            {
                string paramName = $"@excludeId{i}";
                excludeConditions.Add($"id != {paramName}");
                parameters.Add(new MySqlParameter(paramName, excludeIds[i]));
            }
            string excludeIdsClause = string.Join(" AND ", excludeConditions);
            ...
            }, $"SELECT id, dataId, characterName, level FROM characters WHERE characterName LIKE @characterName AND ({excludeIdsClause}) ORDER BY RAND() LIMIT @limit OFFSET @skip",
                parameters.ToArray());
            return result;
        }
        ...
        public override async UniTask<List<SocialCharacterData>> GetFriends(string id, bool readById2, byte state, int skip, int limit)
        {
            ...
            // Batch load all friend character data in a single query using WHERE IN
            List<MySqlParameter> parameters = new List<MySqlParameter>();
            List<string> paramNames = new List<string>();
            for (int i = 0; i < characterIds.Count; i++)
            {
                string paramName = $"@friendId{i}";
                paramNames.Add(paramName);
                parameters.Add(new MySqlParameter(paramName, characterIds[i]));
            }
            string inClause = string.Join(",", paramNames);
            ...
            await ExecuteReader((reader) =>
            {
                while (reader.Read())
                {
                    SocialCharacterData socialCharacterData = new SocialCharacterData();
                    socialCharacterData.id = reader.GetString(0);
                    socialCharacterData.dataId = reader.GetInt32(1);
                    socialCharacterData.characterName = reader.GetString(2);
                    socialCharacterData.level = reader.GetInt32(3);
            ...
            }, $"SELECT id, dataId, characterName, level FROM characters WHERE id IN ({inClause})",
                parameters.ToArray());
