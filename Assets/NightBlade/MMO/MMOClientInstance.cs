...
namespace NightBlade.MMO
{
    ...
    public partial class MMOClientInstance : MonoBehaviour
    {
        ...
        [HideInInspector] public bool connectionSettingsFoldout = true;
        [HideInInspector] public bool networkSettingsFoldout = false;
        [HideInInspector] public bool connectionStatusFoldout = true;
        [HideInInspector] public bool clientActionsFoldout = false;
        ...
        private void Awake()
        {
            if (Singleton != null)
            {
                // If we already have a singleton, destroy this duplicate
                Destroy(gameObject);
                return;
            }
        ...
        private UniTaskVoid OnRequestSelectCharacter(ResponseHandlerData responseHandler, AckResponseCode responseCode, ResponseSelectCharacterMessage response, string characterId, ResponseDelegate<ResponseSelectCharacterMessage> callback)
        {
            ...
            if (responseCode == AckResponseCode.Success)
            {
                ...
                // LOG: Character selection response details for debugging
                Debug.Log($"[MMOClientInstance] Character selection response - Address: '{response.networkAddress}', Port: {response.networkPort}, InstanceId: '{response.instanceId}', MapName: '{response.mapName}'");

                // NOTE: The existing UI system (UIMmoCharacterList.OnRequestedSelectCharacter)
                // will automatically call StartMapClient with these details, so we don't
                // need to manually start the client here.
            }
            return default;
        }
#endregion
