    ...
    public class MMOServerInstance : MonoBehaviour
    {
        ...
        public CentralNetworkManager CentralNetworkManager
        {
            get
            {
                ...
                if (centralNetworkManager == null)
                    centralNetworkManager = FindAnyObjectByType<CentralNetworkManager>();
        ...
        public MapSpawnNetworkManager MapSpawnNetworkManager
        {
            get
            {
                ...
                if (mapSpawnNetworkManager == null)
                    mapSpawnNetworkManager = FindAnyObjectByType<MapSpawnNetworkManager>();
        ...
        public MapNetworkManager MapNetworkManager
        {
            get
            {
                ...
                if (mapNetworkManager == null)
                    mapNetworkManager = FindAnyObjectByType<MapNetworkManager>();
        ...
        [HideInInspector] public bool networkManagersFoldout = true;
        [HideInInspector] public bool databaseConfigFoldout = true;
        [HideInInspector] public bool protocolSettingsFoldout = true;
        [HideInInspector] public bool editorSettingsFoldout = false;
        [HideInInspector] public bool serverStatusFoldout = true;
        [HideInInspector] public bool serverControlsFoldout = false;
        [HideInInspector] public bool diagnosticsFoldout = false;
        ...
        private void Awake()
        {
            ...
            // Setup custom database client ONLY if we're actually using it
            if (useCustomDatabaseClient)
            {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private async void StartServers()
        {
            ...
            // Ensure GameInstance exists
            var gameInstance = GameInstance.EnsureExists();
            if (gameInstance == null)
            {
                Debug.LogError("[MMOServerInstance] GameInstance could not be created. Server startup failed.");
                return;
            }
            ...
            // Initialize guild data safely
            if (GameInstance.Singleton?.SocialSystemSetting != null)
            {
            ...
            {
                Debug.LogWarning("[MMOServerInstance] SocialSystemSetting not available. Guild features may not work properly.");
            }
                ...
                _startingMapServer)
            {
                ...
                // FIX: Even if database server IS running in same process, we need client mode
                // so it can connect to itself via localhost (loopback)
