...
namespace NightBlade.AudioManager
{
    public class AudioSourceSetter : AudioComponent
    {
        ...
        private void Awake()
        {
            ...
            if (playOnAwake && (playMode == PlayMode.PlayClipAtPoint || _cacheAudioSource != null))
                Play();
        ...
        public void Play()
        {
            ...
            // No random clips, try to use clip from audio source (only for PlayClipAtAudioSource mode)
            if (clip == null && playMode == PlayMode.PlayClipAtAudioSource)
            {
                if (_cacheAudioSource == null || _cacheAudioSource.clip == null)
                    return;
                clip = _cacheAudioSource.clip;
            }

            // Don't play if no clip is available
            if (clip == null)
                return;
