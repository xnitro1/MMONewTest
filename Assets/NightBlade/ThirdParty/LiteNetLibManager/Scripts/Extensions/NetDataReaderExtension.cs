...
namespace LiteNetLib.Utils
{
    public static class NetDataReaderExtension
    {
        ...
        /// <summary>
        /// Reads an interned string from the network packet.
        /// </summary>
        /// <param name="reader">The NetDataReader to read from</param>
        /// <returns>The interned string read from the packet</returns>
        public static string GetInternedString(this NetDataReader reader)
        {
            string value = reader.GetString();
            return NetworkStringCache.Intern(value);
        }

        /// <summary>
        /// Reads an array of interned strings from the network packet.
        /// </summary>
        /// <param name="reader">The NetDataReader to read from</param>
        /// <returns>The interned string array read from the packet</returns>
        public static string[] GetInternedStringArray(this NetDataReader reader)
        {
            ushort length = reader.GetUShort();
            if (length == 0) return null;

            string[] values = new string[length];
            for (int i = 0; i < length; i++)
            {
                values[i] = reader.GetInternedString();
            }
            return values;
        }
        #endregion
