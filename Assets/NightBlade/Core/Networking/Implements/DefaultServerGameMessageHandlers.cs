...
namespace NightBlade
{
    public partial class DefaultServerGameMessageHandlers : MonoBehaviour, IServerGameMessageHandlers
    {
        ...
        public void NotifyRewardExp(long connectionId, RewardGivenType givenType, int exp)
        {
            ...
            // Use batching if available, otherwise fall back to direct sending
            var networkManager = GetComponent<BaseGameNetworkManager>();
            if (networkManager != null && networkManager.MessageBatcher != null)
            {
                this.NotifyRewardExpBatched(connectionId, givenType, exp, MessagePriority.High);
            }
            else
            {
        ...
        public void NotifyRewardGold(long connectionId, RewardGivenType givenType, int gold)
        {
            ...
            if (networkManager != null && networkManager.MessageBatcher != null)
            {
                this.NotifyRewardGoldBatched(connectionId, givenType, gold, MessagePriority.High);
            }
            else
            {
        ...
        public void NotifyRewardItem(long connectionId, RewardGivenType givenType, int dataId, int amount)
        {
            ...
            if (networkManager != null && networkManager.MessageBatcher != null)
            {
                this.NotifyRewardItemBatched(connectionId, givenType, dataId, amount, MessagePriority.High);
            }
            else
            {
        ...
        public void NotifyRewardCurrency(long connectionId, RewardGivenType givenType, int dataId, int amount)
        {
            ...
            if (networkManager != null && networkManager.MessageBatcher != null)
            {
                this.NotifyRewardCurrencyBatched(connectionId, givenType, dataId, amount, MessagePriority.High);
            }
            else
            {
