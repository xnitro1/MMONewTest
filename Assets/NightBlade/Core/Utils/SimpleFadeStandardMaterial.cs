...
namespace UtilsComponents
{
    public class SimpleFadeStandardMaterial : MonoBehaviour
    {
        ...
        // Cache original material colors for alpha blending with MaterialPropertyBlocks
        ...
        void SetMaterialsAlpha(float alpha)
        {
            foreach (Renderer renderer in CacheRenderers)
            {
                MaterialPropertyBlockPool.SetProperties(renderer, block => {
                    // For alpha fading, we need to preserve original RGB but modify alpha
                    // Since we can't read from MaterialPropertyBlock, we read from the material once and cache
                    if (!originalColors.ContainsKey(renderer))
                    {
                        originalColors[renderer] = renderer.sharedMaterial?.color ?? Color.white;
                    }

                    Color originalColor = originalColors[renderer];
                    block.SetColor("_Color", new Color(originalColor.r, originalColor.g, originalColor.b, alpha));
                });
            }
        }
