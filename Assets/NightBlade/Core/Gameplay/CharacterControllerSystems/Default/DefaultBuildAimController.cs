...
namespace NightBlade
{
    public class DefaultBuildAimController : MonoBehaviour, IBuildAimController
    {
        ...
        public virtual void Init()
        {
            ...
            _physicFunctions = new PhysicFunctions(128); // 3D-only
        }
        ...
        public void FindAndSetBuildingAreaByAxes(Vector2 aimAxes)
        {
            if (UIBlockController.IsBlockController() || UIBlockActionController.IsBlockController())
                return;
            Vector3 raycastPosition = EntityTransform.position + (GameplayUtils.GetDirectionByAxes(_gameplayCameraController.CameraTransform, aimAxes.x, aimAxes.y) * ConstructingBuildingEntity.BuildDistance) + Vector3.up;
        ...
        public void FindAndSetBuildingAreaByForwardAxes()
        {
            if (UIBlockController.IsBlockController() || UIBlockActionController.IsBlockController())
                return;
            Vector3 raycastPosition = EntityTransform.position + (EntityTransform.forward * ConstructingBuildingEntity.BuildDistance * consoleDistanceRate) + Vector3.up;
            LoopSetBuildingArea(_physicFunctions.RaycastDown(raycastPosition, CurrentGameInstance.GetBuildLayerMask(), 100f, QueryTriggerInteraction.Collide));
        }
